...
ancestry_plot_data <- marginaleffects::datagrid(
...
ancestry_lm_specs <- expand_grid(
  e_name = c("oily_fish", "N3FA", "fish_oil_touchscreen", "fish_oil_verbal", "Omega_3_pct"),
  y_name = c("Omega_3_pct", "hscrp_log", "tg_log", "ldl_statinadj", "apoB_statinadj", "hdl")
) %>%
  filter(e_name != y_name)

run_ancestry_lm <- function(e, y) {
  ancestry_phenos$e <- ancestry_phenos[[e]]
  ancestry_phenos$y <- ancestry_phenos[[y]]
  ancestry_lm_str <- paste0("y ~ e + ", 
                            paste0(all_covars, collapse=" + "), 
                            " + ancestry * e")
  ancestry_lm_fit <- lm(as.formula(ancestry_lm_str), data = ancestry_phenos)
  ...
  ancestry_plot_data <- marginaleffects::datagrid(
    model = ancestry_lm_fit, 
    e = seq(min(ancestry_phenos[[e]], na.rm = TRUE),
                   max(ancestry_phenos[[e]], na.rm = TRUE), length.out = 10),
    ancestry = na.omit(unique(ancestry_phenos$ancestry))
  ) %>%
    mutate(pred = predict(ancestry_lm_fit, newdata = .))
...
a <- ancestry_lm_specs %>%
  rowwise() %>%
  mutate(pred_data = list(run_ancestry_lm(e_name, y_name))) %>%
  unnest(pred_data)

a %>%
  filter(ancestry %in% c("EUR", "AFR")) %>%
  filter(e_name == "Omega_3_pct") %>%
  ggplot(aes(x = e, y = pred, color = ancestry)) +
  geom_line() +
  facet_wrap(vars(y_name), scales = "free")
  # facet_wrap(vars(e_name, y_name), scales = "free")

ancestry_phenos %>% 
  filter(ancestry %in% c("EUR", "AFR")) %>%
         # Omega_3_pct > 2, Omega_3_pct < 10) %>%
  ggplot(aes(x = oily_fish, y = ldl_statinadj, color = ancestry)) +
  geom_smooth()

summary(lm(ldl_statinadj ~ Omega_3_pct * ancestry + age + age_squared + sex + ageBySex_std + ac + income*ancestry + education*ancestry + fresh_fruit*ancestry + raw_veg*ancestry + cooked_veg + alcohol, data = ancestry_phenos))
```
...
fish_oil_med_obj <- test_mediation(
  "fish_oil_touchscreen", "Omega_3_pct", "hscrp_log", all_covars, bl_phenos
)
summary(fish_oil_med_obj)

tg_med_obj <- test_mediation(
  "oily_fish", "Omega_3_pct", "tg_log", all_covars, bl_phenos
)
summary(tg_med_obj)
ldl_med_obj <- test_mediation(
  "oily_fish", "Omega_3_pct", "ldl_statinadj", all_covars, bl_phenos
)
summary(ldl_med_obj)
ldl_med_bmiAdj_obj <- test_mediation(
  "oily_fish", "Omega_3_pct", "ldl_statinadj", c(all_covars, "bmi"), bl_phenos
)
summary(ldl_med_bmiAdj_obj)

dha_med_obj <- test_mediation(
  "oily_fish", "DHA_pct", "hscrp_log", all_covars, bl_phenos
)
summary(dha_med_obj)
bl_phenos$EPA_pct <- bl_phenos$Omega_3_pct - bl_phenos$DHA_pct
epa_med_obj <- test_mediation(
  "oily_fish", "EPA_pct", "hscrp_log", all_covars, bl_phenos
)
summary(epa_med_obj)
```
...
gwis_variables <- c("id", 
                    gwis_bms, 
                    c(fish_variables, "fish_oil", "n3fa_ffq"), 
                    pufa_metabolites, 
...
gwis_df <- phenos %>%
  ...
  mutate(fish_oil = fish_oil_touchscreen,
         n3fa_ffq = as.vector(scale(oily_fish) + 0.5 * scale(fish_oil_touchscreen)))
...
saveRDS(bl_phenos, "../data/processed/manuscript/baseline_phenos.rds")
saveRDS(bl_lm_res_df, "../data/processed/manuscript/bl_lm_res_df.rds")
saveRDS(pheno_corr_df, "../data/processed/manuscript/pheno_corr_df.rds")
saveRDS(ancestry_plot_data, "../data/processed/manuscript/ancestry_plot_data.rds")
saveRDS(list(main = main_med_obj, unadj = unadj_med_obj, nc = neg_control_obj),
        "../data/processed/manuscript/med_obj_list.rds")
```
