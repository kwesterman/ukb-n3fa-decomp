...
make_manhattan(fish_crp_gwis, "robust_P_int", threshold = 5e-8) +
...
make_manhattan(fish_n3_gwis, "robust_P_int", threshold = 5e-8) +
...
make_manhattan(n3_crp_gwis, "robust_P_int", threshold = 5e-8) +
...
gene_summary_df <- bind_rows(list(
  full = fish_crp_genes,
  upstream = fish_n3_genes,
  downstream = n3_crp_genes
), .id = "pathway") %>%
...
phenos <- readRDS("../data/processed/manuscript/baseline_phenos.rds") %>%  # Already covariate-imputed and subsetted to baseline instance
  mutate(id = as.character(id))
...
upstream_rep_df <- replication_export_tbl %>%
  filter(rsid_type == "primary", 
         pathway == "upstream") %>%
  left_join(
    select(fish_n3_gwis, SNP, af_ukb = AF, beta_ukb = `Beta_G-oily_fish`),
    by = c("rsid" = "SNP")
  ) %>%
...
downstream_rep_df <- replication_export_tbl %>%
  filter(rsid_type == "primary", 
         pathway == "downstream") %>%
  left_join(
    select(n3_crp_gwis, SNP, af_ukb = AF, beta_ukb = `Beta_G-Omega_3_pct`),
    by = c("rsid" = "SNP")
  ) %>%
  left_join(wghs_rep_df, by = c("rsid")) %>%
  filter(e == "N3") %>%
