...
make_int_effect_chr_plot <- function(all_maf_ss_df, e, top_rsid) {
  beta_vec <- all_maf_ss_df[[paste0("Beta_G-", e)]]
  ylims <- median(beta_vec, na.rm = TRUE) + c(-5, 5) * mad(beta_vec, na.rm = TRUE)
  all_maf_ss_df %>%
    mutate(int_estimate = !!sym(paste0("Beta_G-", e)),
...
make_strat_plot <- function(top_snp, e, y, df) {
  df %>%
    ...
    labs(x = "Genotype", y = "Mean of Y (95% CI)", 
         title = "Stratified interaction plot for top SNP")
}
...
run_gene_follow_up <- function(gene, e, y, base_covars = covar_sets$ffqAdj,
  ...
  strat_plt <- make_strat_plot(top_snp, e_regression, y, regression_df) 
  print(strat_plt)
  ...
  int_effect_chr_plt <- make_int_effect_chr_plot(all_maf_sumstats_df, e, top_snp)
  ...
  top_cond_sumstats_df <- all_maf_cond_sumstats_df %>%
    ...
    head()
  top_cond_sumstats_df %>%
    kable(caption = "Top variants in conditional analysis")
}
...
fetch_top_rsid <- function(gene) {
  f <- list.files(path = "../data/processed/gene_followup/", 
                  pattern = paste0(".*", tolower(gene), "_top_rsid.txt"),
                  full.names = TRUE)
  scan(f, what = character(), quiet = TRUE)
}
